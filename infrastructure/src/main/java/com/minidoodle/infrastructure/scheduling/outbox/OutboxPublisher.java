...
/**
 * Drains {@code outbox_events}: fetches a small unpublished batch and
 * publishes each row through {@link PublishOutboxEventUseCase} — a
 * separate Spring bean, so each call runs in its own transaction rather
 * than being bypassed by self-invocation. If one row's processing throws,
 * rows already published earlier in this run stay committed (each was its
 * own transaction); the loop stops for this tick and the next
 * {@code fixedDelay} run picks up where it left off.
 *
 * <p>Also owns {@code outbox.lag}: a pull-based gauge reading
 * {@link #lagSeconds}, refreshed on every {@link #drain()} run rather than
 * computed directly by the gauge — the query result (oldest-first, per
 * the repository's contract) is already in hand from draining, so there's
 * no reason for the gauge's scrape-time read to issue a second query.
 */
@Component
